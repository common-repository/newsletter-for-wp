...
class NL4WP_NewsLetter {
	 ...
	 * @var string
	 */
	protected $lists_transient_name = 'nl4wp_newsletter_lists';

	/**
	 * @var string
	 */
	protected $list_counts_transient_name = 'nl4wp_list_counts';
	...
	public function get_lists( $force_fallback = false ) {
		...
		// transient was empty, get lists from NewsLetter
		$api = nl4wp('api');
		...
		foreach ( $lists_data as $list_data ) {
			// create local object
			$list = new NL4WP_NewsLetter_List( $list_data->id, $list_data->name, $list_data->web_id );
			...
			if( $list_data->stats->grouping_count > 0 ) {
				...
				if ( $groupings_data ) {
					$list->groupings = array_map( array( 'NL4WP_NewsLetter_Grouping', 'from_data' ), $groupings_data );
				}
			}
		...
		if ( $merge_vars_data ) {
			foreach ( $merge_vars_data as $list ) {
				...
				$lists["{$list->id}"]->merge_vars = array_map( array( 'NL4WP_NewsLetter_Merge_Var', 'from_data' ), $list->merge_vars );
			}
		}
	...
	public function get_list( $list_id, $force_fallback = false ) {
		...
		return new NL4WP_NewsLetter_List( '', 'Unknown List' );
	}
	...
	public function get_subscriber_counts() {
		...
		if ( is_array( $lists ) ) {
			...
			/**
			 * Filters the cache time for NewsLetter lists configuration. Defaults to 1200.
			 *
			 * @since 2.0
			 * @param int $seconds
			 */
