...
class NL4WP_Admin {
	...
	public function listen_for_actions() {
		...
		if( ! $this->is_user_authorized() || ! isset( $_REQUEST['_nl4wp_action'] ) ) {
			return false;
		}
		...
		$redirect_url = remove_query_arg( '_nl4wp_action' );
	...
	private function init_upgrade_routines() {
		...
		if( ! $previous_version || version_compare( NL4WP_VERSION, $previous_version, '<=' ) ) {
			return false;
		}
	...
	*/
	public function enqueue_assets() {
		...
		// only load asset files on the NewsLetter for WordPress settings pages
	...
	public function show_generals_setting_page() {
		...
		$connected = ( nl4wp('api')->is_connected() );
		$lists = $this->newsletter->get_lists();
