...
class NL4WP_Forms_Admin {
	...
	public function enqueue_assets( $suffix, $page = '' ) {
		...
		wp_localize_script( 'nl4wp-forms-admin', 'nl4wp_forms_i18n', array(
			...
			//'chooseField'   => __( "Choose a NewsLetter field to add to the form", 'newsletter-for-wp' ),
			...
			'value'  => __( "Initial Value", 'newsletter-for-wp' ),
	...
	public function process_add_form() {
		...
		$form_data = stripslashes_deep( $_POST['nl4wp_form'] );
	...
	public function save_form( $data ) {
		...
		if( ! empty( $data['ID'] ) ) {
			...
			// check if attempted post is of post_type `nl4wp-form`
			if( ! is_object( $post ) || $post->post_type !== 'nl4wp-form' ) {
				wp_nonce_ays( '' );
				return 0;
			}
	...
	public function process_save_form( ) {
		...
		$this->messages->flash( __( "<strong>Success!</strong> Form successfully saved.", 'newsletter-for-wp' ) . sprintf( ' <a href="%s">', $previewer->get_preview_url() ) . __( 'Preview form', 'newsletter-for-wp' ) . '</a>' );
	}
	...
	public function prepare_form_preview() {
		$form_id = (int) $_POST['nl4wp_form_id'];
		$previewer = new NL4WP_Form_Previewer( $form_id, true );
