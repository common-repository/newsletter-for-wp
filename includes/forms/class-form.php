...
class NL4WP_Form {
	...
	public static function get_instance( $form_id = 0 ) {
		...
		if( $form_id instanceof WP_Post ) {
			...
			if( empty( $form_id ) ) {
				$form_id = (int) get_option( 'nl4wp_default_form_id', 0 );
			}
		}
	...
	public function __construct( $id ) {
		...
		if( ! is_object( $post ) || ! isset( $post->post_type ) || $post->post_type !== 'nl4wp-form' ) {
			$message = sprintf( __( 'There is no form with ID %d, perhaps it was deleted?', 'newsletter-for-wp' ), $id );
	...
	public function get_html( $element_id = 'nl4wp-form', array $config = array() ) {
		$element = new NL4WP_Form_Element( $this, $element_id, $config );
	...
	public function validate() {
		 ...
		 * @param NL4WP_Form $form
		 */
		$this->errors = (array) apply_filters( 'nl4wp_form_errors', $errors, $form );
	...
	protected function parse_request_data( NL4WP_Request $request ) {
		 ...
		 */
		$data = (array) apply_filters( 'nl4wp_form_data', $data, $form );
