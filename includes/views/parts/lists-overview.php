			...
			if ( ! empty( $list->merge_vars ) ) { ?>
				...
				<table class="widefat striped">
					...
					<?php foreach ( $list->merge_vars as $merge_var ) { ?>
						<tr title="<?php printf( __( '%s (%s) with field type %s.', 'newsletter-for-wp' ), esc_html( $merge_var->name ), esc_html( $merge_var->tag ), esc_html( $merge_var->field_type ) ); ?>">
			...
			if ( ! empty( $list->groupings ) ) { ?>
				...
				<table class="widefat striped">
					...
					<?php foreach ( $list->groupings as $grouping ) { ?>
						<tr title="<?php esc_attr( printf( __( '%s (ID: %s) with type %s.', 'newsletter-for-wp' ), $grouping->name, $grouping->id, $grouping->field_type ) ); ?>">
