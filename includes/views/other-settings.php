 ...
 * @param array $opts
 */
function __nl4wp_usage_tracking_setting( $opts ) {
	?>
	<div class="medium-margin">
	<h3><?php _e( 'Usage Tracking', 'newsletter-for-wp' ); ?></h3>
	<p>
		<label>
			<?php /* hidden input field to send `0` when checkbox is not checked */ ?>
			<input type="hidden" name="nl4wp[allow_usage_tracking]" value="0" />
			<input type="checkbox" name="nl4wp[allow_usage_tracking]" value="1" <?php checked( $opts['allow_usage_tracking'], 1 ); ?>>
			...
			<a href="https://nl4wp.com/kb/what-is-usage-tracking/#utm_source=wp-plugin&utm_medium=newsletter-for-wp&utm_campaign=settings-page" target="_blank">
...
add_action( 'nl4wp_admin_other_settings', '__nl4wp_usage_tracking_setting', 70 );
?>
<div id="nl4wp-admin" class="wrap nl4wp-settings">
	...
	<div class="row">
		...
		<div class="main-content col col-4">
			...
			<!-- Debug Log -->
			<div class="medium-margin">
				<h3><?php _e( 'Debug Log', 'newsletter-for-wp' ); ?> <input type="text" id="debug-log-filter" class="regular-text" placeholder="<?php esc_attr_e( 'Filter..', 'newsletter-for-wp' ); ?>" style="float: right;"/></h3>
				...
				<form method="post">
					...
					<p>
						<input type="submit" class="button" value="<?php esc_attr_e( 'Empty Log', 'newsletter-for-wp' ); ?>" />
					</p>
				</form>
				...
				if( $log->level > 200 ) {
					...
					echo  sprintf( __( 'Would you like to <a href="%s">log all events</a> instead?', 'newsletter-for-wp' ), 'https://nl4wp.com/kb/how-to-enable-log-debugging/' );
					echo '</p>';
				}
				?>
