...
class NL4WP_Visitor_Tracking {
	...
	public function add_hooks() {
		add_action( 'nl4wp_form_subscribed', array( $this, 'on_form_success' ) );
	}

	/**
	 * @hooked `nl4wp_form_subscribed`
	 * @param NL4WP_Form $form
	 */
	public function on_form_success( NL4WP_Form $form ) {
		$this->save( $form->data );
	}
	...
	public function save( array $data ) {
		 ...
		 * @param int $timestamp
		 */
		$expiration_time = apply_filters( 'nl4wp_cookie_expiration_time', $timestamp );
	...
	public function load() {
		...
		if( empty( $this->data ) ) {
			if( ! empty( $_COOKIE[ self::COOKIE_NAME ] ) ) {
				...
				if( is_array( $data ) ) {
					$this->data = nl4wp_sanitize_deep( $data );
				}
