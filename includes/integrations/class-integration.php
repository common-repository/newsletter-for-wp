...
abstract class NL4WP_Integration {
	...
	public function get_checkbox_html() {
		...
		ob_start();
		?>
		<!-- NewsLetter for WordPress v<?php echo NL4WP_VERSION; ?> - https://nl4wp.com/ -->

		<?php do_action( 'nl4wp_integration_before_checkbox_wrapper', $this ); ?>
		<?php do_action( 'nl4wp_integration_'. $this->slug .'_before_checkbox_wrapper', $this ); ?>

		<p class="nl4wp-checkbox nl4wp-checkbox-<?php echo esc_attr( $this->slug ); ?>">
			<label>
		...
		<?php do_action( 'nl4wp_integration_after_checkbox_wrapper', $this ); ?>
		<?php do_action( 'nl4wp_integration_'. $this->slug .'_after_checkbox_wrapper', $this ); ?>
	...
	protected function subscribe( $email, array $merge_vars = array(), $related_object_id = 0 ) {
		...
		// validate lists
		if( empty( $lists ) ) {
			$this->get_log()->warning( sprintf( '%s > No NewsLetter lists were selected', $this->name ) );
			return false;
		}

		/**
		 * Filters the final merge variables before the request is sent to NewsLetter, for all integrations.
		 *
		 * @param array $merge_vars
		 * @param NL4WP_Integration $integration
		 */
		$merge_vars = (array) apply_filters( 'nl4wp_integration_merge_vars', $merge_vars, $integration );

		/**
		 * Filters the final merge variables before the request is sent to NewsLetter, for a specific integration.
		 *
		 ...
		 * @param NL4WP_Integration $integration
		 */
		$merge_vars = (array) apply_filters( 'nl4wp_integration_' . $slug . '_merge_vars', $merge_vars, $integration );
		$email_type = nl4wp_get_email_type();
				

		// create field map
		$map = new NL4WP_Field_Map( $merge_vars, $lists );
		...
		if ( ! $result && $api->has_error() ) {
			...
			if( $api->get_error_code() === 214 ) {
				...
				$this->get_log()->error( sprintf( '%s > NewsLetter API Error: %s', $this->name, $api->get_error_message() ) );
			}
		 ...
		 * @param int $related_object_id
		 */
		do_action( 'nl4wp_integration_subscribed', $integration, $email, $merge_vars, $related_object_id );
