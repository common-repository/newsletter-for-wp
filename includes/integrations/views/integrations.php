 ...
 * @ignore
 */
function __nl4wp_integrations_table( $integrations ) {
	?>
		...
		<?php foreach( $integrations as $integration ) {
			...
			<tr style="<?php if( ! $installed ) { echo 'opacity: 0.4;'; } ?>">
				...
				<td>
					...
					<?php
					if( $installed ) {
						printf( '<strong><a href="%s" title="%s">%s</a></strong>', add_query_arg( array( 'integration' => $integration->slug ) ), __( 'Configure this integration', 'newsletter-for-wp' ), $integration->name );
					} else {
	...
	<div class="main-content row">
		...
		<div class="col col-4">
			...
			<form action="<?php echo admin_url( 'options.php' ); ?>" method="post">
				...
				<?php __nl4wp_integrations_table( $enabled_integrations ); ?>

				<div class="medium-margin"></div>

				<h3><?php _e( 'Available integrations', 'newsletter-for-wp' ); ?></h3>
				<?php __nl4wp_integrations_table( $available_integrations ); ?>
